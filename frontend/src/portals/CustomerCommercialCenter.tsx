...
import { AlertTriangle, Banknote, Check, Download, FileSignature, FileText, ReceiptText, X } from 'lucide-react';
import { decideCustomerProposal, downloadCustomerCommercialPdf, type CustomerCommercialProposal, type CustomerCommercialSummary } from '../services/customerPortalApi';
...
export default function CustomerCommercialCenter({ data, token, onChanged }: { data: CustomerCommercialSummary; token: string; onChanged: (proposal: CustomerCommercialProposal) => void }) {
  ...
  return <section className="customer-commercial-center">
    ...
    {tab === 'proposals' && <div className="customer-commercial-grid">{data.proposals.map((item) => <article className="customer-commercial-card" key={item.id}>
      ...
      <footer><button onClick={() => void downloadCustomerCommercialPdf(token, 'proposals', item.id, item.number).catch((cause) => setError(cause instanceof Error ? cause.message : 'PDF indirilemedi.'))}><Download size={15} /> PDF</button>{item.canDecide && <><button className="reject" onClick={() => setDecision({ proposal: item, accepted: false })}><X size={15} /> Reddet</button><button className="accept" onClick={() => setDecision({ proposal: item, accepted: true })}><Check size={15} /> Onayla</button></>}</footer>
