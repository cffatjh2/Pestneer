  ...
  return <section className="role-surface employee-work-orders-page"><div className="role-section-title"><div><p>SAHA PROGRAMI</p><h2>İş Emirlerim</h2></div><div className="employee-work-actions"><button onClick={() => setRouteOpen(true)}><Route size={16} /> Günün Rotası</button>{options.canSelfSchedule && <button onClick={() => setSelfModal(true)}><CalendarPlus size={16} /> Kendime İş Planla</button>}<button className="icon-button" onClick={() => void load()}><RefreshCw size={16} /></button></div></div>
    ...
    {loading ? <div className="field-loading"><RefreshCw className="spin-icon" size={26} /><span>İş emirleri yükleniyor…</span></div> : orders.length === 0 ? <div className="field-empty-work"><CalendarPlus size={24} /><div><strong>Atanmış iş emri bulunmuyor</strong><span>Yeni bir iş atandığında müşteri, şube, konum ve zaman bilgileri burada görünür.</span></div></div> : <div className="employee-work-list">{orders.map((order) => { const report = reportByOrder.get(order.recordId); const hasActiveSession = order.visitSessions.some((session) => session.employeeAccountId === accountId && session.status === 'Active'); const hasCompletedPart = order.visitSessions.some((session) => session.employeeAccountId === accountId && session.status === 'Completed'); const activeTeam = order.visitSessions.filter((session) => session.status === 'Active'); return <article key={order.recordId} className={`employee-work-card work-${order.technicalStatus.toLowerCase()}`}><div className="employee-work-card-top"><div><span>{order.id}</span><strong>{order.client}</strong><small>{order.branch}</small></div><em>{order.status}</em></div><div className="employee-work-meta"><span><Clock3 size={15} />{order.date} · {order.time}</span><a href={order.branchMapUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(order.branchAddress)}`} target="_blank" rel="noreferrer"><MapPin size={15} />{order.branchAddress}</a></div><div className="employee-work-service"><strong>{order.service}</strong><span>{visitLabel(order.visitType)}</span></div>{order.notes && <p>{order.notes}</p>}{order.completionNote && <div className="employee-completion-note"><CheckCircle2 size={16} /><span>{order.completionNote}</span></div>}
      ...
      <div className="employee-work-card-actions">
        {['Planned','Paused'].includes(order.technicalStatus) && <><button className="role-primary-button" onClick={() => void start(order)}><Play size={16} /> {order.technicalStatus === 'Paused' ? 'Ziyarete Devam Et' : 'İlaçlamayı Başlat'}</button><button className="employee-report-button" onClick={() => setVisitActionOrder(order)}><SlidersHorizontal size={16} /> Ziyaret işlemleri</button></>}
        {order.technicalStatus === 'InProgress' && hasCompletedPart && !hasActiveSession && <><span className="work-completed-label"><CheckCircle2 size={16} /> Saha payınız tamamlandı</span><button className="employee-report-button" onClick={() => setReporting(order)}><FileCheck2 size={16} /> EK-1 formunu aç</button><button className="employee-report-button" onClick={() => setActivationOrder(order)}><FilePlus2 size={16} /> Aktivasyon listesi</button></>}
        {order.technicalStatus === 'InProgress' && !hasActiveSession && !hasCompletedPart && <><button className="role-primary-button" onClick={() => void start(order)}><Play size={16} /> Devam Eden Ziyarete Katıl</button><button className="employee-report-button" onClick={() => setVisitActionOrder(order)}><SlidersHorizontal size={16} /> Ziyaret işlemleri</button></>}
        {order.technicalStatus === 'InProgress' && hasActiveSession && <><button className="role-primary-button" onClick={() => setReporting(order)}><FilePlus2 size={16} /> {report ? 'EK-1 formunu aç' : 'EK-1 formu oluştur'}</button><button className="employee-report-button" onClick={() => setActivationOrder(order)}><FileCheck2 size={16} /> Aktivasyon listesi</button><button className="employee-report-button" onClick={() => setVisitActionOrder(order)}><SlidersHorizontal size={16} /> Ziyaret işlemleri</button></>}
        {order.technicalStatus === 'Completed' && <><span className="work-completed-label"><CheckCircle2 size={16} /> Tamamlandı</span><button className="employee-report-button" onClick={() => setReporting(order)}><FileCheck2 size={16} /> {report ? 'EK-1 formunu görüntüle' : 'EK-1 formu oluştur'}</button><button className="employee-report-button" onClick={() => setActivationOrder(order)}><FilePlus2 size={16} /> Aktivasyon listesi</button></>}
      </div></article>; })}</div>}
