...
function FormFields({ mode, accounts }: { mode: Mode; accounts: SystemAccount[] }) {
  ...
  return <div className="system-admin-fields">{mode === 'company' && <><label>Firma adı<input name="companyName" required /></label><label>Firma kodu<input name="companyCode" required /></label></>}{mode === 'customer' && <><label>Müşteri / marka adı<input name="customerName" required /></label><label>Müşteri kodu<input name="customerCode" placeholder="Boşsa otomatik" /></label></>}<label>{mode === 'company' ? 'Firma sahibi adı soyadı' : mode === 'employee' ? 'Ad soyad' : mode === 'admin' ? 'Sistem yöneticisi' : 'Müşteri yetkilisi'}<input name="name" required /></label><label>{mode === 'company' ? 'Firma sahibi giriş e-postası' : 'E-posta'}<input name="email" type="email" autoComplete="email" required /></label><label>Telefon<input name="phone" /></label><label>Geçici şifre<input name="password" type="password" minLength={8} autoComplete="new-password" required /></label>{mode === 'company' && <label>Geçici şifre tekrarı<input name="passwordConfirmation" type="password" minLength={8} autoComplete="new-password" required /></label>}{mode === 'employee' && <><label>Rol<select name="role"><option value="Technician">Teknisyen</option><option value="OperationsManager">Operasyon yöneticisi</option><option value="Administrator">Yönetici</option></select></label><label className="system-admin-check"><input name="canSelfSchedule" type="checkbox" /> Kendi işini planlayabilsin</label></>}{mode === 'customer' && <><label>İl<input name="city" /></label><label>İlçe<input name="district" /></label><label className="wide">Adres<input name="address" /></label><label className="wide">Google Haritalar bağlantısı<input name="mapUrl" type="url" /></label></>}</div>;
}
