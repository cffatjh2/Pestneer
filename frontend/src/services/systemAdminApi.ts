...
export const updateSystemCompany = (token: string, companyId: string, input: { companyName: string; companyCode: string; reportNotificationEmail?: string; isActive?: boolean }) => request<{ message: string; id: string; legalName: string; code: string; isActive: boolean; reportNotificationEmail?: string }>(`/api/system-control/companies/${companyId}`, token, { method: 'PUT', body: JSON.stringify(input) });
export const deleteSystemCompany = (token: string, companyId: string) => request<{ message: string; id: string }>(`/api/system-control/companies/${companyId}`, token, { method: 'DELETE' });
