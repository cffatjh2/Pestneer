...
import { Copy, Download, DoorOpen, Eye, Grid3X3, LayoutTemplate, Map, Minus, MousePointer2, Plus, Redo2, RotateCcw, Save, Shapes, Square, Trash2, Type, Undo2, X } from 'lucide-react';
...
function SitePlanEditor({ locations, plan, onClose, onSave }: { locations: QualityLocation[]; plan?: SitePlanRecord; onClose: () => void; onSave: (input: SaveSitePlanInput) => Promise<void> }) {
  ...
  const addElement = (type: SitePlanElement['type'], width = 240, height = 130) => {
    ...
    apply([...elements, base]); setSelectedId(base.id);
  };
  const addStation = (equipmentTypeId = activeEquipmentId) => {
    ...
    const station: SitePlanElement = { id: uid(), type: 'station', x: 560, y: 330, width: 34, height: 34, rotation: 0, strokeWidth: 2, equipmentTypeId, stationNumber: `${equipment.code}${String(nextSequence).padStart(2, '0')}` };
    apply([...elements, station]); setSelectedId(station.id);
  };
  ...
  const duplicateSelected = () => { if (!selected) return; const copy = { ...selected, id: uid(), x: selected.x + 24, y: selected.y + 24 }; apply([...elements, copy]); setSelectedId(copy.id); };
