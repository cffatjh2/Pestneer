...
import {
  Copy, Download, DoorOpen, Eye, Grid3X3, LayoutTemplate, Map, Minus, MousePointer2, Plus, QrCode,
  Redo2, RotateCcw, Save, Shapes, Share2, Square, Trash2, Type, Undo2, X, Image as ImageIcon,
  Sliders, UploadCloud, Layers, FileImage, Maximize2, Move
} from 'lucide-react';
...
const emptyCanvas = (): SitePlanCanvas => ({
  width: 1200,
  height: 720,
  equipmentTypes: defaultEquipmentTypes.map((item) => ({ ...item })),
  elements: [],
...
function SitePlanEditor({ locations, plan, onClose, onSave }: { locations: QualityLocation[]; plan?: SitePlanRecord; onClose: () => void; onSave: (input: SaveSitePlanInput) => Promise<void> }) {
  ...
  const [backgroundOpacity, setBackgroundOpacity] = useState<number>(initialCanvas.backgroundOpacity ?? 0.85);
  const [backgroundX, setBackgroundX] = useState<number>(initialCanvas.backgroundX ?? 0);
  const [backgroundY, setBackgroundY] = useState<number>(initialCanvas.backgroundY ?? 0);
  const [backgroundWidth, setBackgroundWidth] = useState<number>(initialCanvas.backgroundWidth ?? 1200);
  const [backgroundHeight, setBackgroundHeight] = useState<number>(initialCanvas.backgroundHeight ?? 720);
  ...
  const handleOverlayImageUpload = async (file?: File) => {
    if (!file) return;
    try {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          const w = 360;
          const h = Math.round((img.height / img.width) * 360);
          const imageElem: SitePlanElement = {
            id: uid(),
            type: 'image',
            x: 420,
            y: 200,
          ...
          apply([...elements, imageElem]);
          setSelectedId(imageElem.id);
      ...
      setError(uploadError instanceof Error ? uploadError.message : 'Görsel öğesi eklenemedi.');
    }
  };
  ...
  const submit = async () => {
    ...
    if (elements.length === 0 && !backgroundImage) return setError('Krokiye en az bir alan, ekipman noktası veya mimari plan görseli ekleyin.');
  ...
  return <div className="site-plan-editor-layer"><div className="site-plan-editor-shell">
    <header><div><span className="site-plan-editor-icon"><Map /></span><div><p className="eyebrow">DİJİTAL KROKİ STÜDYOSU</p><h2>{plan ? 'Yerleşim planını düzenle' : 'Yeni ekipman yerleşim planı'}</h2><p>A4 yatay çıktı · mimari plan entegrasyonu · otomatik lejant · revizyon kontrollü PDF</p></div></div><button className="icon-button" onClick={onClose}><X /></button></header>
    ...
    <div className="site-plan-workspace">
      <aside className={`site-plan-tools ${mobilePanel === 'tools' ? 'mobile-panel-active' : ''}`}>
        
        <div className="blueprint-upload-card">
            ...
            {backgroundImage && (
              <div className="blueprint-active-controls">
                <div className="blueprint-slider-row">
                  <span>Opaklık: %{Math.round(backgroundOpacity * 100)}</span>
                  <input type="range" min="0.1" max="1" step="0.05" value={backgroundOpacity} onChange={(e) => setBackgroundOpacity(Number(e.target.value))} />
                </div>
                <div className="blueprint-quick-btns">
                  <button type="button" onClick={() => { setBackgroundX(0); setBackgroundY(0); setBackgroundWidth(1200); setBackgroundHeight(720); }} title="Tam ekrana sığdır"><Maximize2 size={13} /> Sığdır</button>
                  <button type="button" className="btn-blueprint-remove" onClick={() => setBackgroundImage(null)}><Trash2 size={13} /> Kaldır</button>
                </div>
              </div>
            )}
          </div>
        ...
        <h3><Shapes /> Çizim araçları</h3>
        <div className="site-plan-tool-grid">
          <button onClick={() => addElement('rect', 140, 140)}><Square />Kare</button>
          <button onClick={() => addElement('rect', 280, 160)}><LayoutTemplate />Oda / Alan</button>
          <button onClick={() => addElement('line')}><Minus />Çizgi</button>
          <button onClick={() => addElement('door')}><DoorOpen />Kapı</button>
          <button onClick={() => addElement('text')}><Type />Metin</button>
          <button onClick={() => overlayImageInputRef.current?.click()}><ImageIcon />Görsel Ekle</button>
          <input ref={overlayImageInputRef} type="file" accept="image/png,image/jpeg,image/webp,image/jpg" style={{ display: 'none' }} onChange={(e) => { void handleOverlayImageUpload(e.target.files?.[0]); e.target.value = ''; }} />
        </div>

        <h3><Grid3X3 /> Ekipman noktaları</h3>
        <div className="equipment-palette">{equipmentTypes.map((item) => <button key={item.id} className={activeEquipmentId === item.id ? 'active' : ''} onClick={() => { setActiveEquipmentId(item.id); addStation(item.id); }}><EquipmentSymbol item={item} /><span><b>{item.code}</b>{item.name}</span><Plus /></button>)}</div>
        <button className="add-custom-equipment" onClick={() => setCustomOpen((current) => !current)}><Plus /> Manuel ekipman tanımla</button>
        {customOpen && <form className="custom-equipment-form" onSubmit={addCustomType}><input maxLength={4} placeholder="Kod" value={customType.code} onChange={(event) => setCustomType({ ...customType, code: event.target.value })} /><input placeholder="Ekipman adı" value={customType.name} onChange={(event) => setCustomType({ ...customType, name: event.target.value })} /><select value={customType.shape} onChange={(event) => setCustomType({ ...customType, shape: event.target.value as SitePlanEquipmentShape })}><option value="square">Kare</option><option value="circle">Daire</option><option value="diamond">Baklava</option><option value="star">Yıldız</option><option value="hexagon">Altıgen</option></select><input type="color" value={customType.color} onChange={(event) => setCustomType({ ...customType, color: event.target.value })} /><button>Tanımla</button></form>}
      </aside>

      <main className={`site-plan-stage ${mobilePanel === 'canvas' ? 'mobile-panel-active' : ''}`}>
        <div className="site-plan-toolbar">
          <div>
            <button disabled={!undoStack.current.length} onClick={undo} title="Geri al"><Undo2 /></button>
          ...
          <div className="site-plan-toolbar-right">
            ...
            <select aria-label="Kroki yakınlaştırma" value={zoom} onChange={(event) => setZoom(Number(event.target.value))}><option value="0.28">Sığdır</option><option value="0.4">%40</option><option value="0.55">%55</option><option value="0.72">%72</option><option value="0.9">%90</option><option value="1">%100</option></select>
          </div>
        </div>
        <div className="site-plan-canvas-scroll">
          <div className="site-plan-paper" style={{ width: 1200 * zoom, height: 720 * zoom }}>
            <svg ref={svgRef} viewBox="0 0 1200 720" onPointerDown={() => setSelectedId(null)} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag}>
              {snap && <defs><pattern id="site-grid" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="#E2E8F0" strokeWidth="1" /></pattern></defs>}
              <rect width="1200" height="720" fill={snap ? 'url(#site-grid)' : '#FFFFFF'} />
              {backgroundImage && (
                <image
              ...
              {elements.map((item) => (
                <CanvasElement key={item.id} item={item} equipmentTypes={equipmentTypes} selected={item.id === selectedId} onPointerDown={startDrag} onResizeStart={startResize} />
              ))}
            </svg>
          </div>
        </div>
        <p className="site-plan-touch-hint">Boş alanda kaydırın, öğeyi basılı tutup sürükleyin veya tutamaçlardan boyutlandırın.</p>
      </main>

      <aside className={`site-plan-inspector ${mobilePanel === 'inspector' ? 'mobile-panel-active' : ''}`}>
        <h3><MousePointer2 /> Özellikler</h3>
        {selected ? (
          <ElementInspector element={selected} equipmentTypes={equipmentTypes} onChange={updateSelected} onDelete={removeSelected} onQrPair={() => setQrScannerOpen(true)} />
        ) : backgroundImage ? (
          <div className="background-inspector-card">
            <span className="selected-kind">Arka Plan Kroki / Mimari Plan</span>
            <div className="inspector-slider-row">
              <label>Kroki Opaklığı: %{Math.round(backgroundOpacity * 100)}</label>
              <input type="range" min="0.1" max="1" step="0.05" value={backgroundOpacity} onChange={(e) => setBackgroundOpacity(Number(e.target.value))} />
            </div>
            <div className="inspector-grid">
              <label>X Konumu<input type="number" value={backgroundX} onChange={(e) => setBackgroundX(Number(e.target.value))} /></label>
              <label>Y Konumu<input type="number" value={backgroundY} onChange={(e) => setBackgroundY(Number(e.target.value))} /></label>
              <label>Genişlik<input type="number" value={backgroundWidth} onChange={(e) => setBackgroundWidth(Number(e.target.value))} /></label>
              <label>Yükseklik<input type="number" value={backgroundHeight} onChange={(e) => setBackgroundHeight(Number(e.target.value))} /></label>
            </div>
            <button className="inspector-delete" onClick={() => setBackgroundImage(null)}><Trash2 size={14} /> Arka Plan Krokisini Kaldır</button>
          </div>
        ) : (
          <div className="inspector-empty">
            <MousePointer2 />
            <strong>Bir öğe seçin</strong>
            <span>Konum, ölçü, metin ve numarayı buradan düzenleyin. Sol taraftan kendi mimari planınızı yükleyebilirsiniz.</span>
          </div>
        )}
        <div className="site-plan-summary">
          ...
          {equipmentTypes.map((type) => (
            <div key={type.id}>
              <span><i style={{ background: type.color }} />{type.code} · {type.name}</span>
              <b>{elements.filter((item) => item.type === 'station' && item.equipmentTypeId === type.id).length}</b>
            </div>
          ))}
        </div>
      </aside>
    </div>
    <footer className="site-plan-editor-footer">
      <div>
        <label>Revizyon notu<input value={revisionNote} onChange={(event) => setRevisionNote(event.target.value)} /></label>
      ...
      <button className="primary-button" onClick={() => void submit()} disabled={saving}>
        <Save size={17} />{saving ? 'PDF hazırlanıyor…' : plan ? `R${String(plan.revision + 1).padStart(2, '0')} olarak yayımla` : 'Planı yayımla'}
      </button>
    </footer>
...
function MiniPlan({ canvas }: { canvas: SitePlanCanvas }) {
  return (
    <svg viewBox="0 0 1200 720">
      ...
      {canvas.backgroundImage && (
        <image
          ...
          width={canvas.backgroundWidth ?? 1200}
          height={canvas.backgroundHeight ?? 720}
          opacity={canvas.backgroundOpacity ?? 0.85}
          preserveAspectRatio="xMidYMid meet"
        />
      )}
      {canvas.elements.slice(0, 160).map((item) => (
        <CanvasElement key={item.id} item={item} equipmentTypes={canvas.equipmentTypes} selected={false} onPointerDown={() => undefined} onResizeStart={() => undefined} />
      ))}
    </svg>
  );
}
...
function elementLabel(type: SitePlanElement['type']) { return ({ rect: 'Alan / Dikdörtgen', line: 'Çizgi', door: 'Kapı', text: 'Metin', station: 'Ekipman noktası', image: 'Ekli Görsel' })[type]; }
