...
export default function ServiceReportPrintSheet({ report, accessToken }: { report: ServiceReportRecord; accessToken: string }) {
  ...
  return <article className="official-report-sheet">
    ...
    <section className="official-report-meta"><div><span>İş Emri</span><strong>{report.workOrderNumber}</strong></div><div><span>Uygulama Tarihi</span><strong>{formatDate(report.scheduledAt)}</strong></div><div><span>Başlama / Bitiş</span><strong>{formatTime(report.startedAt || report.scheduledAt)} / {formatTime(report.completedAt || (report.status === 'Finalized' ? report.finalizedAt || report.updatedAt : undefined))}</strong></div><div><span>Doğrulama</span><strong>{report.verificationCode.slice(0, 13).toUpperCase()}</strong></div></section>
