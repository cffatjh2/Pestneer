...
export default function QualityCenter({ accessToken, mode, onSessionExpired, standalone = false, initialTab = 'trend', canManageLicenses = false }: Props) {
  ...
  return <section className={`quality-center ${standalone ? 'page quality-center-page' : 'quality-center-embedded'}`}>
    ...
    {loading ? <div className="surface quality-loading"><RefreshCw className="spin-icon" /><strong>Kalite kayıtları hazırlanıyor…</strong></div> : <>
      ...
      {tab === 'plans' && <SitePlanCenter accessToken={accessToken} mode={mode} locations={locations} selectedLocationKey={selectedLocationKey} onSessionExpired={onSessionExpired} onCount={setSitePlanCount} onSaved={() => refreshDocuments(accessToken, setDocuments)} />}
      {tab === 'audit' && <AuditPackageCenter accessToken={accessToken} mode={mode} locations={locations} selectedLocationKey={selectedLocationKey} onSessionExpired={onSessionExpired} onCount={setAuditPackageCount} />}
