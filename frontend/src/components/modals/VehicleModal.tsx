  ...
  return <div className="modal-layer" role="dialog" aria-modal="true">
    <div className="modal inventory-action-modal"><div className="modal-header"><div className="employee-modal-heading"><span><CarFront size={20} /></span><div><p className="eyebrow">FİLO YÖNETİMİ</p><h2>{vehicle ? 'Aracı Düzenle' : 'Yeni Araç Tanımla'}</h2><p>Plaka, araç bilgisi ve sorumlu personeli tanımlayın.</p></div></div><button className="icon-button" onClick={onClose}><X size={20} /></button></div>
      <form onSubmit={submit}><div className="form-grid">
        <label>Plaka<input value={form.plate} onChange={(event) => setForm({ ...form, plate: event.target.value })} maxLength={16} required /></label>
        <label>Marka<input value={form.brand} onChange={(event) => setForm({ ...form, brand: event.target.value })} maxLength={80} required /></label>
        <label>Model<input value={form.model} onChange={(event) => setForm({ ...form, model: event.target.value })} maxLength={80} required /></label>
        <label>Model yılı<input type="number" min="1980" max="2100" value={form.modelYear ?? ''} onChange={(event) => setForm({ ...form, modelYear: event.target.value ? Number(event.target.value) : undefined })} /></label>
