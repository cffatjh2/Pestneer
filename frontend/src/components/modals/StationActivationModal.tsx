...
export default function StationActivationModal({ accessToken, order, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getStationActivationByWorkOrder(accessToken, order.recordId), getSitePlans(accessToken)])
      .then(([existing, plans]) => {
        if (!active) return;
  ...
  return <div className="modal-layer"><div className="modal station-activation-modal">
    ...
    {loading ? <div className="inspection-loading">İstasyon listesi hazırlanıyor…</div> : <>
      ...
      <div className="activation-workspace"><aside>{stations.map((station, index) => <button type="button" key={`${station.sitePlanElementId ?? 'manual'}-${index}`} className={`${index === selected ? 'active' : ''} status-${station.deviceStatus.toLowerCase()}`} onClick={() => setSelected(index)}><strong>{station.deviceNumber || `Yeni ${index + 1}`}</strong><small>{station.area || 'Konum girilmedi'}</small><em>{statusLabel(station.deviceStatus)}</em></button>)}{!readOnly && <button type="button" className="activation-add" onClick={() => { setStations((items) => [...items, blankStation()]); setSelected(stations.length); }}><Plus /> İstasyon ekle</button>}</aside>
        {current && <main><div className="activation-fields"><label>İstasyon numarası<input value={current.deviceNumber} disabled={readOnly} onChange={(event) => update({ deviceNumber: event.target.value.toUpperCase() })} placeholder="M 01" /></label><label>Konum / alan<input value={current.area} disabled={readOnly} onChange={(event) => update({ area: event.target.value })} placeholder="Üretim çıkışı" /></label><label>Ekipman kodu<input value={current.deviceType} disabled={readOnly} onChange={(event) => update({ deviceType: event.target.value.toUpperCase() })} placeholder="M, C, E, G, B" /></label></div>
          ...
          {current.deviceStatus === 'Activity' && <div className="activation-fields"><label>Zararlı / aktivite türü<input value={current.targetPest ?? ''} disabled={readOnly} onChange={(event) => update({ targetPest: event.target.value, activityType: event.target.value })} placeholder="Ev faresi, karasinek…" /></label><label>Görülen / yakalanan adet<input type="number" min="1" value={current.caughtCount || ''} disabled={readOnly} onChange={(event) => update({ caughtCount: Number(event.target.value) })} /></label></div>}
          {current.deviceStatus === 'Inaccessible' && <label className="activation-wide">Ulaşılamama nedeni<textarea value={current.inaccessibilityReason ?? ''} disabled={readOnly} onChange={(event) => update({ inaccessibilityReason: event.target.value })} /></label>}
