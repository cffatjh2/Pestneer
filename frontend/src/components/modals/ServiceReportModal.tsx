...
export default function ServiceReportModal({ accessToken, order, existing, previousReport, companyName, operatorName, vehicleStockItems = [], readOnly = false, onClose, onSave, onAddManualStock }: Props) {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-label="Saha hizmet raporu"><div className="modal service-report-modal station-report-modal">
    ...
    {false ? <div className="station-inspection-stage">
      ...
      {planLoading ? <div className="inspection-loading">Kroki ve istasyon listesi hazırlanıyor…</div> : <div className="inspection-layout">
        ...
        {currentStation && <main className="inspection-card"><header><div><p>{currentStation.deviceType} EKİPMAN NOKTASI</p><h3>{currentStation.deviceNumber || 'Yeni istasyon'}</h3><span>{currentStation.area || 'Alan bilgisi bekleniyor'}</span></div><b>{stationIndex + 1}/{stations.length}</b></header>
          ...
          {currentStation.deviceStatus === 'Activity' && <ActivityFieldsWithVision accessToken={accessToken} station={currentStation} stockItems={vehicleStockItems} disabled={readOnly} onChange={(patch) => updateStation(stationIndex, patch)} />}
...
function ReportDetails({ catalog, form, setField, products, updateProduct, setProducts, stations, vehicleStockItems, readOnly, setSignatureTarget, onAddManualStock }: { catalog: ServiceReportCatalog; form: FormState; setField: <K extends keyof FormState>(key: K, value: FormState[K]) => void; products: ReportProductInput[]; updateProduct: (index: number, patch: Partial<ReportProductInput>) => void; setProducts: Dispatch<SetStateAction<ReportProductInput[]>>; stations: ReportStationInput[]; vehicleStockItems: VehicleStockCheck['items']; readOnly: boolean; setSignatureTarget: (value: 'manager' | 'customer') => void; onAddManualStock?: () => void }) {
  return <div className="service-report-details"><section className="report-form-section"><header><span>1</span><div><strong>Firma ve uygulama bilgileri</strong><small>Resmi EK-1 alanları</small></div></header><div className="form-grid report-form-grid"><Field label="Uygulayıcı firma" value={form.firmName} onChange={(value) => setField('firmName', value)} disabled={readOnly} /><Field label="Mesul müdür" value={form.responsibleManager ?? ''} onChange={(value) => setField('responsibleManager', value)} disabled={readOnly} /><Field label="Firma adresi" value={form.firmAddress ?? ''} onChange={(value) => setField('firmAddress', value)} disabled={readOnly} wide /><Field label="Firma telefonu" value={form.firmPhone ?? ''} onChange={(value) => setField('firmPhone', value)} disabled={readOnly} /><Field label="İzin tarih / sayısı" value={form.permissionNumber ?? ''} onChange={(value) => setField('permissionNumber', value)} disabled={readOnly} /><Field label="Ekip sorumlusu" value={form.teamManager ?? ''} onChange={(value) => setField('teamManager', value)} disabled={readOnly} /><CatalogMultiSelect label="Hedef zararlı" options={catalog.pestTypes} value={form.targetPests ?? ''} disabled={readOnly} onChange={(value) => setField('targetPests', value)} /><Field label="Alan (m²)" value={form.areaSquareMeters} onChange={(value) => setField('areaSquareMeters', value)} disabled={readOnly} type="number" /><CatalogSelectField label="Mahal türü" options={catalog.residenceTypes} value={form.residenceType ?? ''} disabled={readOnly} onChange={(value) => setField('residenceType', value)} /><CatalogMultiSelect label="İş türü" options={catalog.workTypes} value={form.workType ?? ''} disabled={readOnly} onChange={(value) => setField('workType', value)} wide /><CatalogMultiSelect label="Güvenlik önlemleri" options={catalog.safetyMeasures} value={form.safetyMeasures ?? ''} disabled={readOnly} onChange={(value) => setField('safetyMeasures', value)} wide /></div></section>
    {stations.length > 0 && <section className="report-form-section legacy-station-summary"><header><span>2</span><div><strong>Önceki birleşik kaydın istasyon özeti</strong><small>Bu bölüm yalnız eski raporların geriye dönük görüntülenmesi için korunur</small></div></header><div className="report-entry-table-wrap"><table className="report-entry-table"><thead><tr><th>No</th><th>Alan</th><th>Durum</th><th>Zararlı</th><th>Adet</th><th>Ürün / Sarf</th></tr></thead><tbody>{stations.map((station) => <tr key={`${station.sitePlanElementId}-${station.deviceNumber}`}><td><strong>{station.deviceNumber}</strong></td><td>{station.area}</td><td>{statusLabel(station.deviceStatus)}</td><td>{station.targetPest || '—'}</td><td>{station.caughtCount || '—'}</td><td>{station.appliedProductName ? `${station.appliedProductName} · ${station.appliedAmount} ${station.appliedUnit}` : station.replacementProductName ? `${station.replacementProductName} · ${station.replacementQuantity} ${station.replacementUnit}` : '—'}</td></tr>)}</tbody></table></div></section>}
    <section className="report-form-section"><header><span>3</span><div><strong>Ürün ve sarf tüketimleri</strong><small>Araç stoğundan seçilir; rapor onayında stoktan otomatik düşer</small></div>{!readOnly && <div className="report-stock-actions"><button onClick={() => setProducts((current) => [...current, blankProduct()])}><Plus size={15} /> Stoktan seç</button>{onAddManualStock && <button onClick={onAddManualStock}><PackageCheck size={15} /> Manuel sarf ekle</button>}</div>}</header><div className="report-product-list">{products.map((product, index) => <article key={index}><div className="form-grid report-form-grid"><StockSelect label="Araç stoğundan ürün / sarf" value={product.vehicleStockItemId} stockItems={vehicleStockItems} disabled={readOnly} onChange={(stock) => updateProduct(index, { vehicleStockItemId: stock?.vehicleStockItemId, productName: stock?.productName ?? '', unit: stock ? usageUnits(stock.unit)[0] : 'Adet', licenseNumber: stock?.licenseNumber ?? '', licenseDocumentId: stock?.licenseDocumentId })} /><Field label="Ruhsat / ürün bilgisi" value={product.licenseNumber ?? ''} onChange={(value) => updateProduct(index, { licenseNumber: value, licenseDocumentId: undefined })} disabled={readOnly || Boolean(product.licenseDocumentId)} placeholder={product.vehicleStockItemId ? 'Ürüne bağlı ruhsat bulunamadı' : 'Sarf malzemesi için gerekmez'} /><CatalogSelectField label="Kullanım yeri / yöntemi" options={catalog.applicationMethods} value={product.applicationMethod ?? ''} disabled={readOnly} onChange={(value) => updateProduct(index, { applicationMethod: value })} /><Field label="Etken madde / sarf türü" value={product.activeIngredient ?? ''} onChange={(value) => updateProduct(index, { activeIngredient: value })} disabled={readOnly} /><Field label="Kullanılan miktar" value={String(product.amountUsed || '')} onChange={(value) => updateProduct(index, { amountUsed: Number(value) })} disabled={readOnly} type="number" /><label>Birim<select value={product.unit} disabled={readOnly || !product.vehicleStockItemId} onChange={(event) => updateProduct(index, { unit: event.target.value })}>{usageUnits(vehicleStockItems.find((item) => item.vehicleStockItemId === product.vehicleStockItemId)?.unit).map((unit) => <option key={unit}>{unit}</option>)}</select></label></div>{product.licenseDocumentId && <small className="report-license-linked"><FileCheck2 size={14} /> Ürüne bağlı güncel ruhsat EK-1 formuna otomatik eklendi.</small>}{!readOnly && products.length > 1 && <button className="report-product-remove" onClick={() => setProducts((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={14} /> Kalemi kaldır</button>}</article>)}</div></section>
...
function CatalogMultiSelect({ label, options, value, disabled, onChange, wide = false }: { label: string; options: string[]; value: string; disabled: boolean; onChange: (value: string) => void; wide?: boolean }) {
  const raw = value.split(/[,;]+/).map((item) => item.trim()).filter(Boolean);
  const selected = raw.filter((item) => options.includes(item));
  ...
  const emit = (known: string[], other = otherValue, keepOther = otherSelected) => onChange([...known, ...(keepOther ? [`Diğer: ${other.trim()}`] : [])].join(', '));
  const toggle = (item: string) => emit(selected.includes(item) ? selected.filter((entry) => entry !== item) : [...selected, item]);
  return <fieldset className={`work-type-multi ${wide ? 'form-field-wide' : ''}`} disabled={disabled}><legend>{label} <small>Birden fazla seçilebilir</small></legend><div>{options.map((item) => <button type="button" aria-pressed={selected.includes(item)} className={selected.includes(item) ? 'active' : ''} key={item} onClick={() => toggle(item)}><Check size={14} />{item}</button>)}<button type="button" aria-pressed={otherSelected} className={otherSelected ? 'active' : ''} onClick={() => emit(selected, '', !otherSelected)}><Plus size={14} />Diğer</button></div>{otherSelected && <input value={otherValue} disabled={disabled} onChange={(event) => emit(selected, event.target.value, true)} placeholder={`${label} için diğer değer`} />}</fieldset>;
}
...
function ActivityFieldsWithVision({ accessToken, station, stockItems, disabled, onChange }: { accessToken: string; station: ReportStationInput; stockItems: VehicleStockCheck['items']; disabled: boolean; onChange: (patch: Partial<ReportStationInput>) => void }) {
  return <><ActivityFields station={station} stockItems={stockItems} disabled={disabled} onChange={onChange} /><PestneerVisionAnalyzer accessToken={accessToken} value={station.pestObservations} disabled={disabled} onApply={(pestObservations, summary) => onChange({ pestObservations, caughtCount: summary.total, targetPest: summary.dominantPest || station.targetPest, hasActivity: summary.total > 0 || station.hasActivity, activityType: summary.total > 0 ? (station.activityType || 'Capture') : station.activityType, deviceStatus: summary.total > 0 ? 'Activity' : station.deviceStatus })} /></>;
}
...
function createInitialForm(order: WorkOrder, report: ServiceReportRecord | undefined, companyName: string): FormState { return { firmName: report?.firmName ?? companyName, firmAddress: report?.firmAddress ?? '', firmPhone: report?.firmPhone ?? '', firmWeb: report?.firmWeb ?? '', responsibleManager: report?.responsibleManager ?? '', permissionNumber: report?.permissionNumber ?? '', teamManager: report?.teamManager ?? '', targetPests: normalizeCatalogList(report?.targetPests ?? '', pestTypes), residenceType: normalizeCatalogValue(report?.residenceType ?? 'İşyeri', residenceTypeOptions), areaSquareMeters: report?.areaSquareMeters?.toString() ?? '', workType: normalizeCatalogList(report?.workType ?? order.service, workTypeOptions), consumables: report?.consumables ?? '', safetyMeasures: normalizeCatalogList(report?.safetyMeasures ?? 'Uygulama alanı bilgilendirildi, Kişisel koruyucu donanım kullanıldı', safetyMeasureOptions), applicationSummary: report?.applicationSummary ?? order.completionNote ?? '', findings: report?.findings ?? '', correctiveActions: report?.correctiveActions ?? '', recommendations: report?.recommendations ?? order.recommendation ?? '', customerRepresentativeName: report?.customerRepresentativeName ?? '', managerSignatureData: report?.managerSignatureData ?? '', customerSignatureData: report?.customerSignatureData ?? '', additionalEmailRecipients: report?.additionalEmailRecipients ?? [] }; }
