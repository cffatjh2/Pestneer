...
import { ReportConflictError, type ReportProductInput, type ReportStationInput, type ServiceReportRecord, type UpsertServiceReportInput } from '../../services/serviceReportApi';
...
type Props = {
  ...
  onSave: (input: UpsertServiceReportInput, photos: File[]) => Promise<void>;
};
...
export default function ServiceReportModal({ accessToken, order, existing, previousReport, companyName, operatorName, vehicleStockItems = [], readOnly = false, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [accessToken, draftReady, existing?.stations.length, order.branchId, order.customerId, restoredDraft]);
  ...
  useEffect(() => {
    ...
    getLocalReportDraft(order.recordId).then((draft) => {
      ...
      setPhotos(draft.photos.map((photo) => new File([photo.blob], photo.name, { type: photo.type, lastModified: photo.lastModified })));
  ...
  const buildInput = (finalize: boolean, forceOverwrite = false): UpsertServiceReportInput => ({ ...form, areaSquareMeters: form.areaSquareMeters ? Number(form.areaSquareMeters) : undefined, baseUpdatedAt, forceOverwrite, finalize, stations: stations.filter((item) => item.deviceNumber.trim() || item.area.trim()), products: products.filter((item) => item.productName.trim()) });
  ...
  const selectStationStatus = (value: string) => {
    ...
    updateStation(stationIndex, value === 'Activity' ? {
      deviceStatus: value, hasActivity: true,
    ...
    } : { deviceStatus: value, hasActivity: false, caughtCount: 0 });
  };
...
function ReportDetails({ form, setField, products, updateProduct, setProducts, stations, vehicleStockItems, readOnly, setSignatureTarget }: { form: FormState; setField: <K extends keyof FormState>(key: K, value: FormState[K]) => void; products: ReportProductInput[]; updateProduct: (index: number, patch: Partial<ReportProductInput>) => void; setProducts: Dispatch<SetStateAction<ReportProductInput[]>>; stations: ReportStationInput[]; vehicleStockItems: VehicleStockCheck['items']; readOnly: boolean; setSignatureTarget: (value: 'manager' | 'customer') => void }) {
  return <div className="service-report-details"><section className="report-form-section"><header><span>1</span><div><strong>Firma ve uygulama bilgileri</strong><small>Resmi EK-1 alanları</small></div></header><div className="form-grid report-form-grid"><Field label="Uygulayıcı firma" value={form.firmName} onChange={(value) => setField('firmName', value)} disabled={readOnly} /><Field label="Mesul müdür" value={form.responsibleManager ?? ''} onChange={(value) => setField('responsibleManager', value)} disabled={readOnly} /><Field label="Firma adresi" value={form.firmAddress ?? ''} onChange={(value) => setField('firmAddress', value)} disabled={readOnly} wide /><Field label="Firma telefonu" value={form.firmPhone ?? ''} onChange={(value) => setField('firmPhone', value)} disabled={readOnly} /><Field label="İzin tarih / sayısı" value={form.permissionNumber ?? ''} onChange={(value) => setField('permissionNumber', value)} disabled={readOnly} /><Field label="Ekip sorumlusu" value={form.teamManager ?? ''} onChange={(value) => setField('teamManager', value)} disabled={readOnly} /><Field label="Hedef zararlı" value={form.targetPests ?? ''} onChange={(value) => setField('targetPests', value)} disabled={readOnly} placeholder="Hamamböceği, kemirgen, karasinek…" /><Field label="Alan (m²)" value={form.areaSquareMeters} onChange={(value) => setField('areaSquareMeters', value)} disabled={readOnly} type="number" /><Field label="Mahal türü" value={form.residenceType ?? ''} onChange={(value) => setField('residenceType', value)} disabled={readOnly} /><Field label="İş türü" value={form.workType ?? ''} onChange={(value) => setField('workType', value)} disabled={readOnly} /><TextField label="Sarf malzemeleri" value={form.consumables ?? ''} onChange={(value) => setField('consumables', value)} disabled={readOnly} /><TextField label="Güvenlik önlemleri" value={form.safetyMeasures ?? ''} onChange={(value) => setField('safetyMeasures', value)} disabled={readOnly} /></div></section>
    ...
    <section className="report-form-section"><header><span>3</span><div><strong>Genel uygulama ürünleri</strong><small>İstasyon dışı biyosidal uygulamalar</small></div>{!readOnly && <button onClick={() => setProducts((current) => [...current, blankProduct()])}><Plus size={15} /> Ürün ekle</button>}</header><div className="report-product-list">{products.map((product, index) => <article key={index}><div className="form-grid report-form-grid"><StockSelect label="Araç stoğundan ürün" value={product.vehicleStockItemId} stockItems={vehicleStockItems} disabled={readOnly} onChange={(stock) => updateProduct(index, { vehicleStockItemId: stock?.vehicleStockItemId, productName: stock?.productName ?? '', unit: stock ? usageUnits(stock.unit)[0] : 'Mililitre' })} /><Field label="Ruhsat bilgisi" value={product.licenseNumber ?? ''} onChange={(value) => updateProduct(index, { licenseNumber: value })} disabled={readOnly} /><Field label="Uygulama yöntemi" value={product.applicationMethod ?? ''} onChange={(value) => updateProduct(index, { applicationMethod: value })} disabled={readOnly} /><Field label="Etken madde" value={product.activeIngredient ?? ''} onChange={(value) => updateProduct(index, { activeIngredient: value })} disabled={readOnly} /><Field label="Kullanılan miktar" value={String(product.amountUsed)} onChange={(value) => updateProduct(index, { amountUsed: Number(value) })} disabled={readOnly} type="number" /><label>Birim<select value={product.unit} disabled={readOnly || !product.vehicleStockItemId} onChange={(event) => updateProduct(index, { unit: event.target.value })}>{usageUnits(vehicleStockItems.find((item) => item.vehicleStockItemId === product.vehicleStockItemId)?.unit).map((unit) => <option key={unit}>{unit}</option>)}</select></label></div>{!readOnly && products.length > 1 && <button className="report-product-remove" onClick={() => setProducts((current) => current.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={14} /> Ürünü kaldır</button>}</article>)}</div></section>
    ...
    <section className="report-form-section"><header><span>5</span><div><strong>Dijital onay</strong><small>Uygulayıcı ve müşteri yetkilisi imzası</small></div></header><div className="report-signature-grid"><SignatureCard label="Uygulayıcı / ekip sorumlusu" value={form.managerSignatureData} disabled={readOnly} onClick={() => setSignatureTarget('manager')} /><div className="report-customer-sign"><Field label="Müşteri yetkilisi" value={form.customerRepresentativeName ?? ''} onChange={(value) => setField('customerRepresentativeName', value)} disabled={readOnly} /><SignatureCard label="Müşteri yetkilisi imzası" value={form.customerSignatureData} disabled={readOnly} onClick={() => setSignatureTarget('customer')} /></div></div></section></div>;
}
...
function DamageFields({ station, stockItems, disabled, onChange }: { station: ReportStationInput; stockItems: VehicleStockCheck['items']; disabled: boolean; onChange: (patch: Partial<ReportStationInput>) => void }) {
  ...
  return <section className="inspection-conditional damaged"><h4><Wrench /> Hasarlı ekipman işlemi</h4><label className="inspection-check"><input type="checkbox" checked={replacing} disabled={disabled} onChange={(event) => onChange(event.target.checked ? { replacementQuantity: 1, replacementUnit: 'Adet' } : { replacementQuantity: 0, replacementVehicleStockItemId: undefined, replacementProductName: undefined, replacementUnit: undefined })} /> Yeni istasyon / ekipman yerleştirildi</label>{replacing && <div className="form-grid"><StockSelect label="Yerleştirilen ekipman" value={station.replacementVehicleStockItemId} stockItems={stockItems} disabled={disabled} onChange={(stock) => onChange({ replacementVehicleStockItemId: stock?.vehicleStockItemId, replacementProductName: stock?.productName, replacementUnit: stock ? usageUnits(stock.unit)[0] : 'Adet' })} /><Field label="Miktar" value={String(station.replacementQuantity ?? 1)} onChange={(value) => onChange({ replacementQuantity: Number(value) })} disabled={disabled} type="number" /><label>Birim<select value={station.replacementUnit ?? 'Adet'} disabled={disabled} onChange={(event) => onChange({ replacementUnit: event.target.value })}>{usageUnits(stockItems.find((item) => item.vehicleStockItemId === station.replacementVehicleStockItemId)?.unit ?? 'Adet').map((unit) => <option key={unit}>{unit}</option>)}</select></label></div>}</section>;
}
...
function StatusButton({ icon, label, value, station, disabled, onSelect }: { icon: React.ReactNode; label: string; value: string; station: ReportStationInput; disabled: boolean; onSelect: (value: string) => void }) { return <button className={station.deviceStatus === value ? 'active' : ''} disabled={disabled} onClick={() => onSelect(value)}>{icon}<span>{label}</span></button>; }
